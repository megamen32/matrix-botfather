...
function page() {
  return String.raw`<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Klub control plane</title><style>body{font:16px system-ui;max-width:980px;margin:2rem auto;padding:0 1rem;color:#172033;background:#f5f7fb}textarea,input,select{width:100%;box-sizing:border-box;margin:.35rem 0 .75rem;padding:.5rem}textarea{min-height:8rem}button{padding:.55rem 1rem;margin:.25rem .35rem .25rem 0}article{background:#fff;border:1px solid #ccd3df;border-radius:.55rem;padding:1rem;margin:.8rem 0;box-shadow:0 1px 3px #0001}.status-ok{color:#16733b}.status-error{color:#a12727}.meta{color:#536176}.models{columns:2;padding-left:1.2rem}@media(max-width:700px){.models{columns:1}}</style><h1>Klub control plane</h1><p>Changes are versioned. The club bot applies them after a controlled restart.</p><section id="gen"><article><h2>Unified gen</h2><p>Loading status...</p></article></section><section id="gen-personas"><article><h2>Matrix persona identities</h2><p>Loading identities...</p></article></section><section id="llm"></section><section id="personas"></section><script>
...
async function loadGenPersonas(){try{genPersonas=await api('/api/gen/personas');const create='<article><h2>Matrix persona identities</h2><p class="meta">These identities live in the unified orchestrator; source is fixed to Matrix.</p><label>Matrix user or room ID<input id="gen-new-chat" placeholder="@persona:chat.example or !room:chat.example"></label>'+identityFields('gen-new')+'<button onclick="upsertGenPersona()">Create or replace identity</button></article>';const existing=genPersonas.map((p,i)=>'<article><h3>'+esc(p.chat_id)+'</h3><p class="meta">Confirmed replies: '+esc(p.confirmed_count??0)+'</p>'+identityFields('gen-'+i,p)+'<button onclick="saveGenPersona('+i+')">Save identity</button><button onclick="deleteGenPersona('+i+')">Delete identity</button></article>').join('');document.querySelector('#gen-personas').innerHTML=create+existing;}catch(e){document.querySelector('#gen-personas').innerHTML=errorCard('Matrix persona identities',e);}}
...
async function savePersona(name){await api('/api/personas/'+encodeURIComponent(name),{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({displayname:document.querySelector('#d-'+name).value,prompt:document.querySelector('#p-'+name).value})});await loadLocal();}
